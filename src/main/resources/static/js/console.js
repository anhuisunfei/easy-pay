...
layui.use(["element","okUtils", "table", "countUp"], function () {
    ...
    function statText() {
        okUtils.ajaxCloud({
            url:"/statistic/console",
            success : function(result) {
                showCount(0,result.totalFee);
                showCount(1,1000);
                showCount(2,result.orderCount);
                showCount(3,result.userCount);
            }
        });
    }

    function showCount(i,count){
         var elemNum = $(".stat-text");
         !new countUp({
             target:  elemNum[i],
             endVal: count
         }).start();
